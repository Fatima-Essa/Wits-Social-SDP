...
import { IoIosPaperPlane as TweetIcon } from "react-icons/io";
import { BsVectorPen as PenIcon } from "react-icons/bs";
import { GrRobot as BotIcon } from "react-icons/gr";
...
const Header = () => {
  ...
  const uploadTweet = (e) => {
        e.preventDefault();
        const storageRef = ref(storage, `users/${user?.uid}/tweets/${uuid()}`);
        const uploadSingleImage = uploadBytesResumable(storageRef, "https://firebasestorage.googleapis.com/v0/b/witsinsta-f21ed.appspot.com/o/users%2FAGBD3lFe3XTID4cfkzhAe6uTkuX2%2Fposts%2F7d0a5d84-670f-47d3-9be7-328b00a5a0c7?alt=media&token=38cb8d50-56ee-4276-8276-dad59f10830b");
        uploadSingleImage.on(
            "state_changed",
            (snap) => {
                setUploading(true);
            ...
            () => {
                getDownloadURL(uploadSingleImage.snapshot.ref).then(
                    async (downloadURL) => {
                        console.log("File available at", downloadURL);
                        const postDoc = await addDoc(collection(firestore, "tweets"), {
                            caption,
                            createdAt: serverTimestamp(),
                            singleMedia: { src: downloadURL },
                            user: {
  ...
  return (
                ...
                <>
                  <div className="flex items-center text-slate-800 text-2xl gap-3">
                    ...
                    <button onClick={() => setModelOpen(true)}>
                      ...
                      <button onClick={() => setAddTweet(true)}>
                          <PenIcon size={20}/>
                      </button>
                    <NavLink to="/explore">
                      <ExploreIcon size={20} />
                    </NavLink>
                      <NavLink to="/ChatBotPage">
                          <BotIcon size={20} />
                      </NavLink>
                    <NavLink to="/TweetsHome">
                      <TweetIcon size={22} />
                    </NavLink>
                    {user ? (
                        <div
      ...
      {addTweet && (
          <div className="fixed top-0 flex items-center justify-center left-0 w-screen h-screen z-50">
              ...
              <div className="p-3 rounded-lg overflow-hidden">
                  <div className="bg-white rounded-lg">
                      <div className="border-b-2 py-2 text-center">Write Tweet</div>
                      <div className="max-w-[422px] w-full h-full aspect-square flex items-center justify-center">
                          <div className="flex flex-col w-full overflow-hidden items-center justify-between gap-4">
                              {/*{images ? (*/}
                              {/*    <>*/}
